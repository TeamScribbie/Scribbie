...
export const usePhysics = ({
    ...
    player, setPlayer, cagedWords, setCagedWords, fishLogics, setFishLogics, monster,
    ...
    const spawnFish = useCallback(() => {
        ...
        switch (side) {
            case 0: // Left
                position = { x: player.position.x - viewportWidth / 2 - spawnMargin, y: player.position.y + (Math.random() - 0.5) * height };
                velocity = { x: 1, y: 0 };
                break;
            case 1: // Right
                position = { x: player.position.x + viewportWidth / 2 + spawnMargin, y: player.position.y + (Math.random() - 0.5) * height };
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            if (event.code === 'KeyQ') { event.preventDefault(); onVomit(); }
    ...
    useTick(delta => {
        ...
        if (fishLogics) {
            setFishLogics(logics => logics.map(l => { l.update(delta, player.position, logics); return l; }));
        }
        ...
        if (gameState === 'intro') {
            const gravity = 0.6;
            const dropTargetY = height / 2 + (MONSTER_HEIGHT / 2) + 50;
            velY += gravity * delta;
            if (player.position.y >= dropTargetY) {
        ...
        let nextPlayerPos = {
            x: player.position.x + velX * delta,
            y: player.position.y + velY * delta,
        };
        ...
        if (monster && isColliding(nextPlayerPos, playerRadius, monsterBounds)) {
            if (boostInfo.current.isBoosting) {
                if (onMonsterDash) onMonsterDash();
                velX *= -1.5; velY *= -1.5;
        ...
        newCages.forEach(cage => {
            ...
            if (isColliding(nextPlayerPos, playerRadius, cageBounds)) {
                if (cage.isBroken) {
                    ...
                    if (cage.audioUrl) { const sound = new Audio(cage.audioUrl); sound.play().catch(e => console.error("Error playing sound:", e)); }
            ...
            if (monsterBounds && isRectColliding(cageBounds, monsterBounds)) {
                const dx = cage.position.x - monster.position.x; const dy = cage.position.y - monster.position.y;
                const distance = Math.sqrt(dx * dx + dy * dy) || 1;
                const pushX = dx / distance; const pushY = dy / distance;
                const pushStrength = 2; const recoilDampening = -0.8;
                ...
                cage.position.x += pushX * minOverlap; cage.position.y += pushY * minOverlap;
            }
        });
        ...
        newCages.forEach(cage => {
            ...
            cage.velocity.x *= friction; cage.velocity.y *= friction;
            const halfCageW = CAGE_WIDTH / 2; const halfCageH = CAGE_HEIGHT / 2;
        ...
        if (fishLogics) {
            fishLogics.forEach(logic => { if (logic.state.fishToEatId) { fishEatenByOtherFish.add(logic.state.fishToEatId); } });
        }
